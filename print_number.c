...
*--str = '-';
if (!para->minus_flag)
return (func_print_number_right_shift(str, para));
else
return (func_print_number_left_shift(str, para));
}
/**
...
n += _putchar('+');
else if (!para->plus_flag && para->space_flag && !
neg2 && !para->unsign && para->zero_flag)
n += _putchar(' ');
while (i++ < para->width)
n += _putchar(pad_char);
if (neg && pad_char == ' ')
