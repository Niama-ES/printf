...
int print_number(char *str, paras *para)
{
    ...
    if (neg)
        *--str = '-';
    if (!para->minus_flag)
        return (func_print_number_right_shift(str, params));
    else
        return (func_print_number_left_shift(str, params));
}
 /**
 * func_print_number_right_shift - prints a number with options
 * @str: the base number as a string
 * @para: the parameter struct
 *
 * Return: chars printed
...
int func_print_number_right_shift(char *str, paras *para)
{
    ...
    if (neg && i < para->width && pad_char == '0' && !para->minus_flag)
        ...
        neg = 0;
    if((para->plus_flag && !neg2) || (!para->plus_flag && para->space_flag && !neg2))
        i++;
    if (neg && pad_char == '0')
        n += _putchar('-');
    if (para->plus_flag && !neg2 && pad_char == '0' && !para->unsign)
        n += _putchar('+');
    else if (!para->plus_flag && para->space_flag && !neg2 && !para->unsign && para->zero_flag)
        n += _putchar(' ');
    while (i++ < para-> width)
        n += _putchar(pad_char);
    if(neg && pad_char == ' ')
 ...
 /**
 * func_print_number_left_shift - prints a number with options
 * @str: the base number as a string
 * @para: the parameter struct
 *
 * Return: chars printed
